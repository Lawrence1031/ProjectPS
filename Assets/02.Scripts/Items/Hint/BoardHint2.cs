...
public class BoardHint2 : MonoBehaviour
{
    // BoardManager에 대한 참조.
    public BoardManager2 boardManager2;
    // 힌트를 표시할 프리팹에 대한 참조.
    ...
    IEnumerator Start()
    {
        ...
        yield return new WaitUntil(() => boardManager2.IsInitialized);
    ...
    void GenerateHints()
    {
        ...
        foreach (GameObject board in boardManager2.GetAllBoards())
        {
            // 간격을 30%로 조정하기 위해 BoardManager와의 상대 위치를 계산하고 0.3배 함.
            // 이는 힌트를 BoardManager가 생성한 발판에 비례하여 축소
            Vector3 hintPosition = transform.position + (board.transform.position - boardManager2.transform.position) * 0.2f;
